...
class Path :
    ...
    def distance (self) :
        ...
        for x in range(len(self.points)-1):
            result += self.points[x].distance_to(self.points[x+1])
        return result
...
##                   
            ...
            if n == j:
                ...
                new_path.extend(self.ordered[0])
                n = i+1
                flag = False
            elif n<=i:
                if flag:
                    new_path.extend(self.ordered[n])
                    n = n-1
                else:
                    n = n+1                
            else:
