...
class Board : 
    ...
    def fakefloodFill(self, x, y, c, fflood):
        if x != 0:
            left = self.tiles[x-1][y]
            if left not in fflood:
                if left.color == c:
                    fflood.add(left)
                    self.fakefloodFill(left.x,left.y,c,fflood)
        if y != 0:
            up = self.tiles[x][y-1]
            if up not in fflood:
                if up.color == c:
                    fflood.add(up)
                    self.fakefloodFill(up.x,up.y,c,fflood)
        if x != len(self.tiles)-1:
            right = self.tiles[x+1][y]
            if right not in fflood:
                if right.color == c:
                    fflood.add(right)
                    self.fakefloodFill(right.x,right.y,c,fflood)
        if y != len(self.tiles)-1:
            down = self.tiles[x][y+1]
            if down not in fflood:
                if down.color == c:
                    fflood.add(down)
                    self.fakefloodFill(down.x,down.y,c,fflood)
            
    def fakeMove (self,c):
        fflood = self.flooded.copy()        
        for tile in fflood.copy():
            tile.updateColor(c)
            self.fakefloodFill(tile.x,tile.y,c,fflood)
        return fflood
    ...
    def unfloodedneighbors (self,x,y,fflood):
        perm = 0        
        if x != 0:
            left = self.tiles[x-1][y]
            if left not in self.flooded:
    ...
    def greedy1 (self) :
        p = len(self.fakeMove(PINK))
        v = len(self.fakeMove(VIOLET))
        y = len(self.fakeMove(YELLOW))
        r = len(self.fakeMove(RED))
        o = len(self.fakeMove(OLIVE))
        b = len(self.fakeMove(BLUE))
        m = max(p,v,y,r,o,b)
        if p == m:
            return PINK
        if v == m:
